function sendWelcomeEmailOnNewRow(e) {
 ...
 // --- COLUMN MAPPING ---
 // Using your actual column structure:
 // Column H = Date of Birth (index 7)
 // Column I = Gender (index 8)  
 // Column J = Bodyweight lbs (index 9)
 // Column K = Email Address (index 10)
 // Column D = Athlete Name (index 3)
 // Column Y = Total Dead Hang Time (index 24)
 
 const emailColIndex = 10; // Column K
 const nameColIndex = 3;   // Column D
 const timeColIndex = 24;  // Column Y
 ...
 const dob = row[dobColIndex]; // Date of Birth (Date object)
...
// Check if we have valid grip age data
// We need: Date of Birth (to calculate age), Gender, and Bodyweight
const hasGripAgeData = dob && dob instanceof Date && 
                       gender !== undefined && gender !== null && gender !== '' &&
                       weight !== undefined && weight !== null && weight !== '' &&
                       age !== null; // Age calculated successfully from DOB
