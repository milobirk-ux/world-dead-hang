/**
 * WDHC Email Automation Script v2.3 - SIMPLIFIED GRIP AGE CALCULATION
 * 
...
function calculateGripAge(dob, bodyweight, gender, hangTime, gripTraining) {
    ...
    // Grip Training Experience Adjustment - MATCHES FORM OPTIONS EXACTLY
    let experienceAdjustment = 1.0;
    if (gripTraining) {
        const trainingStr = gripTraining.toString().toLowerCase();
        
        // EXACT MATCHES FROM SUBMISSION FORM:
        if (trainingStr.includes('none')) {
            experienceAdjustment = 0.85; // 15% penalty for "None - First time training grip"
        } else if (trainingStr.includes('beginner')) {
            experienceAdjustment = 0.90; // 10% penalty for "Beginner - Less than 6 months"
        } else if (trainingStr.includes('intermediate')) {
            experienceAdjustment = 1.0; // No adjustment for "Intermediate - 6 months to 2 years"
        } else if (trainingStr.includes('advanced')) {
            experienceAdjustment = 1.15; // 15% bonus for "Advanced - 2+ years"
        }
    }
    
    // Combined adjustment (HEIGHT REMOVED AS REQUESTED)
    const adjustedExpectedTime = (baseExpectedTime * weightAdjustment * experienceAdjustment * 0.7) + (baseExpectedTime * 0.3);
    ...
    return {
        ...
        adjustments: {
            ...
            trainingLevel: gripTraining || "Not specified"
        }
    };
}
...
function doGet(e) {
    return ContentService.createTextOutput(JSON.stringify({ 
        status: "WDHC Email Automation v2.3", 
        version: "2.3", 
        features: ["tier_badges", "enhanced_time_parsing", "simplified_grip_age_calculation"],
        grip_age_changes: ["height_removed", "grip_training_matches_form_exactly", "minimum_age_18"],
        grip_training_levels: ["None (-15%)", "Beginner (-10%)", "Intermediate (0%)", "Advanced (+15%)"]
    }))
