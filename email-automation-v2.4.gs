// WDHC Google Apps Script - Complete Form Handler + Email Automation v2.5
// For Custom Form Submissions Sheet with Grip Age Calculation
// Version 2.5: Fixed CORS headers (removed setHeaders), added grip age explanation

// ========== FORM HANDLER ==========
function doGet() {
  return ContentService.createTextOutput(JSON.stringify({
    status: 'OK',
    message: 'WDHC Form Handler v2.5 is running',
    version: '2.5'
...
function triggerEmailAutomation(sheet, row) {
  ...
  // In a real implementation, you might want to use a trigger or queue
  // For now, we'll call the email function directly
  try {
    sendWelcomeEmailOnNewRow({ source: SpreadsheetApp.getActiveSpreadsheet(), changeType: 'INSERT_ROW' });
  } catch (error) {
...
<body>
  ...
  <div class="footer">
    ...
    console.error('Error in sendWelcomeEmailOnNewRow:', error);
  }
}
