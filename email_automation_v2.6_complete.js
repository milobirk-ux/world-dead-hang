...
function calculateGripAge(age, hangTimeSeconds, weight, gender, height, training) {
    ...
    const referenceTime = gender === 'Male' ? 60 : 45;
    if (hangTimeSeconds > referenceTime) {
        const timeBonus = (hangTimeSeconds - referenceTime) / 10;
        gripAge -= timeBonus;
    } else {
        const timePenalty = (referenceTime - hangTimeSeconds) / 10;
        gripAge += timePenalty;
    }
    
    const referenceWeight = gender === 'Male' ? 175 : 140;
    if (weight < referenceWeight) {
        const weightBonus = (referenceWeight - weight) / 10;
        gripAge -= (weightBonus * 0.5);
    } else {
        const weightPenalty = (weight - referenceWeight) / 10;
...
function getGripAgeComparison(gripAge, chronologicalAge) {
    ...
    if (ageDifference < -5) {
        return `<div style="margin: 15px 0; padding: 20px; background: linear-gradient(135deg, #e8f5e9, #c8e6c9); border-radius: 10px; border-left: 4px solid #4caf50;"><strong style="color: #2e7d32;">🎉 Amazing Grip Age!</strong><br>Your grip age is <strong>${absoluteDifference} years younger</strong> than your chronological age.</div>`;
    } else if (ageDifference < 0) {
        return `<div style="margin: 15px 0; padding: 20px; background: #e8f5e9; border-radius: 10px; border-left: 4px solid #4caf50;"><strong style="color: #2e7d32;">👍 Strong Grip!</strong><br>Your grip age is <strong>${absoluteDifference} years younger</strong> than your chronological age.</div>`;
    } else if (ageDifference === 0) {
        return `<div style="margin: 15px 0; padding: 20px; background: #fff3e0; border-radius: 10px; border-left: 4px solid #ff9800;"><strong style="color: #ef6c00;">📊 Solid Foundation!</strong> Your grip age matches your chronological age.</div>`;
    } else if (ageDifference <= 5) {
        return `<div style="margin: 15px 0; padding: 20px; background: #fff3e0; border-radius: 10px; border-left: 4px solid #ff9800;"><strong style="color: #ef6c00;">💪 Room for Growth!</strong><br>Your grip age is <strong>${absoluteDifference} years older</strong> than your chronological age.</div>`;
    } else {
        return `<div style="margin: 15px 0; padding: 20px; background: #ffebee; border-radius: 10px; border-left: 4px solid #f44336;"><strong style="color: #c62828;">🚀 Huge Potential!</strong><br>Your grip age is <strong>${absoluteDifference} years older</strong> than your chronological age.</div>`;
    }
}